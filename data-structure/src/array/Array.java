...
public class Array<E> {// 使用泛型
    private final static int DEFAULT_SIZE = 10;// 默认的数组容量

    private E[] data;// 动态数组的底层容器
    private int size;// 数组的长度

    /**
     * 根据传入的 capacity 定义一个指定容量的数组
     * @param capacity
     */
    ...
    public void add(int index, E e){
        if(index < 0 || this.size < index){
            throw new IllegalArgumentException("添加失败，要求参数 index >= 0 并且 index <= size");
        }
        if(size == data.length){
